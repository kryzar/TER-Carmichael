...
\begin{remarque}
	L'entier $n = 8029$ a la particularité d'être le produit de trois nombres premiers distincts : \[n = 8029 = 7 \cdot 31 \cdot 37.\] Le fait qu'il existe des corps quadratiques $K$ dans lesquels $n$ soit de Carmichael, mais ne le soit pas dans $\Q$, peut se traduire par : existe-t-il un triplet de nombres premiers distincts $p$, $q$ et $r$ pour lesquels \[(pqr)^2 - 1\] soit divisible à la fois par $p-1$, $q-1$ et $r-1$ ? Les diviseurs premiers de $8029$ constituent un tel triplet\footnote{Nous avons exhibé de nombreux autres entiers $n$ produits de trois nombres premiers comme dans cette remarque. Voir la liste d'entiers $n$ donnée dans le fichier \url{https://github.com/kryzar/TER-Carmichael/blob/master/Simulations\%20et\%20resultats/Carmichael\_ideal\_restricted\_2.txt}.}.
\end{remarque}
