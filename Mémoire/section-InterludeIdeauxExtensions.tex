...
Donnons nous \[\Q\subset K \subset L\] une tour de corps de nombres, $I\subset \OK$ un idéal de Carmichael de $K$ et $J \subset \OL$ un idéal de Carmichael de $L$. L'idéal étendu $I\OL$ est-il de Carmichael dans $L$ et l'idéal restreint $J\cap\OK$ est-il de Carmichael dans $K$ ? L'auteur de l'article a déjà répondu à la première partie de la question dans l'exemple 2.6 de l'article, en utilisant le test de Fermat dans les corps quadratiques.
...
\KwOut{couples $(n, K)$ où $K$ est un corps quadratique de $\mathscr{K}$ et $n$ est est un entier de $\mathscr{N}$ de Carmichael dans $K$ mais pas dans $\Q$}
...
Cette algorithme retourne de nombreux résultats. Par exemple $35$ est de Carmichael dans $\Q(\sqrt{11})$ et $8029$ est de Carmichael dans $\OL$. \NTS{lien vers résultats complets} Nous avons tiré les statistiques suivantes.
