...
\begin{proof}
	Supposons par l'absurde que $\p$ est un idéal premier de $A$ et que $\p^n$ divise $I$, autrement dit que l'on a $$I \subset \p^n.$$ Comme $(JJ')B = (JB)(J'B)$ pour tout couple $J, J'$ d'idéaux de $A$, on a $$IB \subset (\p B)^n = \p^n B.$$ On a en particulier l'inclusion $$IB\subset \P^n$$ pour tout idéal premier $\P$ de $\OL$ divisant $\pB$, d'où une contradiction, puis la conclusion.
\end{proof}
\begin{proof}
